#!/usr/bin/env python3
"""Find the latest wandb run for the project.

Usage:
    uv run scripts/get_latest_wandb_run.py [--state running] [--wait 300]

Options:
    --state   Filter by run state (default: running)
    --wait    Max seconds to poll for a new run (default: 0 = no wait)
...
PROJECT = "success-hf/Leggy"


def find_run(api, state):
